import numpy as np

class RewardGenerator:
    def __init__(self, verbose=False):
        self.verbose = verbose
        # self.verbose = False

    def _verbose(self, msg):
        if self.verbose:
            print(msg)

    def __call__(self, obs):
        return self.reward(obs)
    
    def reward(self, obs):
        #input shape: (14,)

        linear_velocity = obs[0]  # linear velocity
        angular_velocity = obs[1]  # angular velocity
        robot_pos = obs[2:4]  # robot x, y position
        # heading_sin = obs[4]  # sin, cos of robot heading relative to goal
        heading_cos = obs[5]  # sin, cos of robot heading relative to goal
        dist_to_obstacles = obs[6:10]  # lidar readings
        # ind_to_obstacles = obs[10:14]  # indices of obstacles in lidar readings
        # self._verbose(f"dist_to_obstacles: {dist_to_obstacles}")

        linear_velocity = obs[0]  # linear velocity
        angular_velocity = obs[1]  # angular velocity
        robot_pos = obs[2:4]  # robot x, y position
        # heading_sin = obs[4]  # sin, cos of robot heading relative to goal
        heading_cos = obs[5]  # sin, cos of robot heading relative to goal
        dist_to_obstacles = obs[6:10]  # lidar readings

        dist_to_goal = np.linalg.norm(robot_pos)

        heading_reward = (heading_cos + 1) / 2

        index_to_closest_obstacle = np.argmin(dist_to_obstacles)
        dist_to_obstacles = dist_to_obstacles[index_to_closest_obstacle]

        if dist_to_obstacles > 0.99:
            obstacle_reward = 0
        else:
            obstacle_reward = 1/((dist_to_obstacles*5)**2 + 0.01) #cap this to 0.01

        # goal_reward = 1/((dist_to_goal*2) + 0.01) #exp #cap this to 0.01
        if dist_to_goal > 0.4:
            goal_reward = -dist_to_goal**2
        elif dist_to_goal < 0.2:
            goal_reward = 1/((dist_to_goal*5)**2 + 0.01) #cap this to 0.01
        else:
            goal_reward = 0.0

        return self.compute(
            linear_velocity, angular_velocity, dist_to_goal,
            heading_reward, index_to_closest_obstacle, obstacle_reward, goal_reward
        )

    def compute(self, linear_velocity, angular_velocity, dist_to_goal,
                heading_reward, index_to_closest_obstacle, obstacle_reward, goal_reward):
        return 0


class RewardGeneratorRightTurn(RewardGenerator):
    def __init__(self, verbose=False):
        super().__init__(verbose)
        # hard code lidar range sectors
        self.sectors_mul = np.array([
            0.2,   # back
            0.2,   # left
            1.0,   # front
            0.5,   # right
        ])

    def compute(self, linear_velocity, angular_velocity, dist_to_goal,
                heading_reward, index_to_closest_obstacle, obstacle_reward, goal_reward):

        obstacle_reward = obstacle_reward * self.sectors_mul[index_to_closest_obstacle]
        if linear_velocity < 0:
            # obstacle_reward = np.sum(obstacle_reward * self.reverse_sectors_mul)
            speed_reward = -0.5
        else:
            # obstacle_reward = np.sum(obstacle_reward * self.sectors_mul)
            if linear_velocity < 0.05:
                speed_reward = 0.1
            else:
                speed_reward = 0.2

        #add reward for turning with slow linear velocity
        if linear_velocity < 0.05:
            angular_reward = 0
        else:
            angular_reward = max(abs(angular_velocity) - 2.0, 0)

        reward = (
            - 9.0 * dist_to_goal
            + 0.5 * goal_reward
            - 0.5 * obstacle_reward
            + 0.3 * speed_reward
            - 0.3 * angular_reward
            + 6.0 * speed_reward * heading_reward
        )

        self._verbose(
              f"Reward: {reward:>10.4f}, "
              f"Distance to Goal: {dist_to_goal:>8.4f}, "
              f"Obstacle Reward: {obstacle_reward:>8.4f}, "
            )
        return reward
    
class RewardGeneratorLeftTurn(RewardGenerator):
    def __init__(self, verbose=False):
        super().__init__(verbose)
        # hard code lidar range sectors
        self.sectors_mul = np.array([
            0.2,   # back
            0.5,   # left
            1.0,   # front
            0.2,   # right
        ])

    def compute(self, linear_velocity, angular_velocity, dist_to_goal,
                heading_reward, index_to_closest_obstacle, obstacle_reward, goal_reward):

        obstacle_reward = obstacle_reward * self.sectors_mul[index_to_closest_obstacle]
        if linear_velocity < 0:
            # obstacle_reward = np.sum(obstacle_reward * self.reverse_sectors_mul)
            speed_reward = -0.5
        else:
            # obstacle_reward = np.sum(obstacle_reward * self.sectors_mul)
            if linear_velocity < 0.05:
                speed_reward = 0.1
            else:
                speed_reward = 0.2

        #add reward for turning with slow linear velocity
        if linear_velocity < 0.05:
            angular_reward = 0
        else:
            angular_reward = max(abs(angular_velocity) - 2.0, 0)

        reward = (
            - 9.0 * dist_to_goal
            + 0.5 * goal_reward
            - 0.5 * obstacle_reward
            + 0.3 * speed_reward
            - 0.3 * angular_reward
            + 6.0 * speed_reward * heading_reward
        )

        self._verbose(
              f"Reward: {reward:>10.4f}, "
              f"Obstacle Reward: {obstacle_reward:>8.4f}, "
              f"Heading Reward: {heading_reward:>8.4f}, "
            )
        return reward
    
class RewardGeneratorSteering(RewardGenerator):
    def __init__(self, verbose=False):
        super().__init__(verbose)
        # hard code lidar range sectors
        self.sectors_mul = np.array([
            0.2,   # back
            0.4,   # left
            1.0,   # front
            0.4,   # right
        ])

    def compute(self, linear_velocity, angular_velocity, dist_to_goal,
                heading_reward, index_to_closest_obstacle, obstacle_reward, goal_reward):

        obstacle_reward = obstacle_reward * self.sectors_mul[index_to_closest_obstacle]
        if linear_velocity < 0:
            # obstacle_reward = np.sum(obstacle_reward * self.reverse_sectors_mul)
            speed_reward = -0.5
        else:
            # obstacle_reward = np.sum(obstacle_reward * self.sectors_mul)
            if linear_velocity < 0.05:
                speed_reward = 0.1
            else:
                speed_reward = 0.2

        #add reward for turning with slow linear velocity
        if linear_velocity < 0.05:
            angular_reward = 0
        else:
            angular_reward = max(abs(angular_velocity) - 2.0, 0)

        reward = (
            - 9.0 * dist_to_goal
            + 0.5 * goal_reward
            - 0.5 * obstacle_reward
            + 0.3 * speed_reward
            - 0.3 * angular_reward
            + 6.0 * speed_reward * heading_reward
        )

        self._verbose(
              f"Reward: {reward:>10.4f}, "
              f"Distance to Goal: {dist_to_goal:>8.4f}, "
              f"Obstacle Reward: {obstacle_reward:>8.4f}, "
            )
        return reward
    
class RewardGeneratorDreamer(RewardGenerator):
    def __init__(self, verbose=False):
        super().__init__(verbose)
        # hard code lidar range sectors
        self.sectors_mul = np.array([
            0.2,   # back
            0.4,   # left
            1.0,   # front
            0.4,   # right
        ])

    def reward(self, obs):
        #input shape: (14,)

        linear_velocity = obs[0]  # linear velocity
        angular_velocity = obs[1]  # angular velocity
        robot_pos = obs[2:4]  # robot x, y position
        # heading_sin = obs[4]  # sin, cos of robot heading relative to goal
        heading_cos = obs[5]  # sin, cos of robot heading relative to goal
        dist_to_obstacles = obs[6:10]  # lidar readings
        # ind_to_obstacles = obs[10:14]  # indices of obstacles in lidar readings
        # self._verbose(f"dist_to_obstacles: {dist_to_obstacles}")

        linear_velocity = obs[0]  # linear velocity
        angular_velocity = obs[1]  # angular velocity
        robot_pos = obs[2:4]  # robot x, y position
        # heading_sin = obs[4]  # sin, cos of robot heading relative to goal
        heading_cos = obs[5]  # sin, cos of robot heading relative to goal
        dist_to_obstacles = obs[6:10]  # lidar readings

        dist_to_goal = np.linalg.norm(robot_pos)

        heading_reward = (heading_cos + 1) / 2

        index_to_closest_obstacle = np.argmin(dist_to_obstacles)
        dist_to_obstacles = dist_to_obstacles[index_to_closest_obstacle]

        if dist_to_obstacles > 0.99:
            obstacle_reward = 0
        elif dist_to_obstacles > 0.2:
            obstacle_reward = 1/((dist_to_obstacles*5)**2 + 0.01) #cap this to 0.01
        else:
            obstacle_reward = 100

        # goal_reward = 1/((dist_to_goal*2) + 0.01) #exp #cap this to 0.01
        if dist_to_goal > 0.4:
            goal_reward = -dist_to_goal**2
        elif dist_to_goal > 0.2:
            goal_reward = 1/((dist_to_goal*5)**2 + 0.01) #cap this to 0.01
        else:
            goal_reward = 50

        return self.compute(
            linear_velocity, angular_velocity, dist_to_goal,
            heading_reward, index_to_closest_obstacle, obstacle_reward, goal_reward
        )

    def compute(self, linear_velocity, angular_velocity, dist_to_goal,
                heading_reward, index_to_closest_obstacle, obstacle_reward, goal_reward):

        obstacle_reward = obstacle_reward * self.sectors_mul[index_to_closest_obstacle]
        if linear_velocity < 0:
            # obstacle_reward = np.sum(obstacle_reward * self.reverse_sectors_mul)
            speed_reward = -0.5
        else:
            # obstacle_reward = np.sum(obstacle_reward * self.sectors_mul)
            if linear_velocity < 0.05:
                speed_reward = 0.1
            else:
                speed_reward = 0.2

        #add reward for turning with slow linear velocity
        if linear_velocity < 0.05:
            angular_reward = 0
        else:
            angular_reward = max(abs(angular_velocity) - 2.0, 0)

        reward = (
            - 9.0 * dist_to_goal
            + 0.5 * goal_reward
            - 0.5 * obstacle_reward
            + 0.3 * speed_reward
            - 0.3 * angular_reward
            + 6.0 * speed_reward * heading_reward
        )

        self._verbose(
              f"Reward: {reward:>10.4f}, "
              f"Distance to Goal: {dist_to_goal:>8.4f}, "
              f"Obstacle Reward: {obstacle_reward:>8.4f}, "
            )
        return reward
