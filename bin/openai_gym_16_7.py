...
class OpenAIGymEnvironment(Supervisor, gym.Env):
    ...
    def step(self, action):
        ...
        #     + 10.0 * linear_velocity * (speed_reward + 0.5 * (abs(heading_diff) < 0.2))
        ...
        observe_value = - 20.0 * dist_to_goal**2 - 100.0 / ((dist_to_obstacle*5)**2 + 0.1)
        ...
        reward = collision * 200.0 + (1.0 - collision) * (
            ...
            + 10.0 * linear_velocity * (speed_reward + 0.5 * (abs(heading_diff) < 0.2))
            - 10.0 * max(abs(angular_velocity) - 1.5, 0)
            - 30.0 * abs(heading_diff)
            - 5e-3 * (self.step_lapsed // 10)
        )
